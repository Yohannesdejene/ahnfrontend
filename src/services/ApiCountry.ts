...
interface CREATE_COUNTRY_DATA_TYPE {
  name: string;
  isoCode: string;
  countryCode: string;
  addedBy: string | null;
...
export async function apiGetCountriesList(): Promise<any> {
  const method = "GET"; // Use GET method
  const url = `${BASE_URL}countries`; // Adjust the endpoint to match your API's URL
