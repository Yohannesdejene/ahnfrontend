...
const ShipmentGroundList: React.FC<GradeDetailProps> = ({ id }) => {
  ...
  const hasAcceptedGroundShipmentPermission = auth.some(
    (permission: any) => permission.code === "ACCEPTED_GROUND_SHIPMENTS",
  );
  const hasArrivingGroundShipmentPermission = auth.some(
    (permission: any) => permission.code === "ARRIVING_GROUND_SHIPMENT",
  );

  const hasArrivedGroundShipmentPermission = auth.some(
    (permission: any) => permission.code === "ARRIVED_GROUND_SHIPMENT",
  );
  const hasDeliveredGroundShipmentPermission = auth.some(
    (permission: any) => permission.code === "DELIVERED_GROUND_SHIPMENT",
  );
  const hasPermission =
    (id === "READY_FOR_PICK_UP" && hasAcceptedGroundShipmentPermission) ||
    (id === "ARRIVING" && hasArrivingGroundShipmentPermission) ||
    (id === "ARRIVED" && hasArrivedGroundShipmentPermission) ||
    (id === "DELIVERED" && hasDeliveredGroundShipmentPermission);
