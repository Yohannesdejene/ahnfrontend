...
const ChangeStatus: React.FC<GradeDetailProps> = ({ id }) => {
  ...
  const hasAirStatusChangesPermission = auth.some(
    (permission: any) => permission.code === "CHANGE_AIR_SHIPMENT_STATUS",
  );
  const hasGroundStatusChangesPermission = auth.some(
    (permission: any) => permission.code === "CHANGE_GROUND_SHIPMENT_STATUS",
  );
  const hasPermission =
    (id === "air" && hasAirStatusChangesPermission) ||
    (id === "ground" && hasGroundStatusChangesPermission);
