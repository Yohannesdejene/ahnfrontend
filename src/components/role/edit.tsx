...
import { updateRole } from "@/store/features/roles/rolesThunk";
...
import { getRoleById } from "@/store/features/roles/rolesSlice";
import {
  setUser,
  setToken,
  setPermissions,
...
const EditRole: React.FC<AddUserProps> = ({ id, setId, setIsDialogOpen }) => {
  ////permission
  ...
  const addUser = async (values: FormData) => {
    ...
    dispatch(updateRole({ id: id, roleData: request_body }))
      .then((data: any) => {
  ...
  useEffect(() => {
    console.log("id-1111111111111111111111111", id);
    console.log("in the use effect effecting ...");
    if (id) {
      dispatch(getRoleById(id))
        .then((data: any) => {
          const roles = data?.payload?.role;
          const codesArray = roles?.Permissions.map((item: any) => item.code);
          // Update the state with the codes as a Set
          setSelectedPermissions(new Set(codesArray));
          methods.reset({
            name: data?.payload?.role?.name || "",
            description: data?.payload?.role?.description || "",
          });
        })
        .catch((error) => {
  ...
  return (
    <>
      <div
        className=" m-0  w-full bg-white   p-5 text-black dark:bg-boxdark dark:text-white"
        style={{ minHeight: "100vh" }}
      >
        {getRoleByIdError && <Alert severity="error">{getRoleByIdError}</Alert>}
        {updateRoleError && <Alert severity="error">{updateRoleError}</Alert>}
        {getRoleByIdLoading ? (
          <div className=" mt-12  justify-center align-middle">
            <LinearProgress
                ...
                <div className="w-full">
                  ...
                  >
                    <div className="container mx-auto p-4">
                      ...
                      <div className="align-center mb-6 mt-2 flex gap-3">
